// media-library.js - Popup window for browsing and selecting media files (optimized dynamic version)
(function() {
    let mediaLibrary = null;
    let currentCallback = null;
    let currentMultiple = false;
    let cachedFiles = null; // cached file list in memory for the session
    ...
    function createMediaLibrary() {
        ...
        modal.innerHTML = `
            ...
            <input type="text" id="mediaSearch" placeholder="Search files..." 
                style="width:100%;padding:8px 12px;margin-bottom:15px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);
            ...
            <div id="mediaGrid" style="flex:1;overflow-y:auto;display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));
                gap:10px;padding:10px;background:rgba(255,255,255,0.02);border-radius:8px;">
    ...
    async function scanFilesOnce() {
        if (isScanning || cachedFiles) return cachedFiles;
        isScanning = true;
        const results = [];
        ...
        for (let i = 1; i <= maxScan; i++) {
            for (const ext of imageExt) {
                const path = `../image/image${i}${ext}`;
                if (await fileExists(path)) {
                    results.push({ name: `image${i}${ext}`, type: 'image' });
                    break;
                }
            }
        }
        for (let i = 1; i <= maxScan; i++) {
            for (const ext of videoExt) {
                const path = `../video/video${i}${ext}`;
                if (await fileExists(path)) {
                    results.push({ name: `video${i}${ext}`, type: 'video' });
                    break;
                }
            }
        }
    ...
    async function showFiles(type = 'image') {
        ...
        let visible = type === 'all' ? files : files.filter(f => f.type === type);
